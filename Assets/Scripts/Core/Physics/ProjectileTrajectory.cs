...
{
    public Vector3[] CalculateBallisticPathPoints(Vector3 initialPos, Vector3 initialVel, Vector3 totalAcc, float timeStep, float maxTime)
    {
       ...
       for (int i = 1; i < maxIterations; i++)
        {
            currentVel = currentVel + totalAcc * timeStep;
            points[i] = points[i-1]  + currentVel * timeStep + 0.5f * totalAcc * Mathf.Pow(timeStep, 2);
        }
